...
export const RoleOptions = ({
  ...
  peerList.forEach(peer => {
    const isAudioOn =
      !!peer.audioTrack && store.tracks[peer.audioTrack]?.enabled;
    const isVideoOn =
      !!peer.videoTrack && store.tracks[peer.videoTrack]?.enabled;
  ...
  if (
    !(canMuteRole || canRemoveRoleFromStage || canRemoveRoleFromRoom) ||
    peerList.length === 0
  ) {
    return null;
  }
      ...
      >
        ...
        {canMuteRole && (
          <>
            <Dropdown.Item
              css={dropdownItemCSS}
              onClick={() => setTrackEnabled("audio", !allPeersHaveAudioOn)}
            >
